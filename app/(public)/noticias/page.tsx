...
export default async function NoticiasPage() {
  ...
  const { data: comunicados } = await (supabase as any)
    .from('comunicados').select('*').eq('es_publico', true).eq('publicado', true).order('created_at', { ascending: false })

  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="border-b-4 border-[#c0392b] pb-6 mb-10">
        <h1 className="text-4xl md:text-5xl font-bold text-[#1a3a5c]">Noticias</h1>
        ...
        : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {comunicados.map((com: any) => (
              <article key={com.id} className="bg-white rounded-xl border border-slate-200 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-200 overflow-hidden">
                {com.imagen_url
                  ? <img src={com.imagen_url} alt={com.titulo} className="w-full h-52 object-cover" loading="lazy" />
                  : <div className="w-full h-52 bg-gradient-to-br from-[#1a3a5c] to-[#2563EB] flex items-center justify-center"><FileText className="w-14 h-14 text-white/20" /></div>
                }
                <div className="p-5">
                  <time className="text-xs text-[#c0392b] font-semibold uppercase tracking-wider">{formatDate(com.created_at)}</time>
                  <h2 className="font-bold text-[#1a3a5c] text-lg mt-1 mb-2 leading-tight line-clamp-2">{com.titulo}</h2>
